...
test_that("rmdiff: unstratified test of rmst difference", {
  ...
  expect_equal(c(round(df1$rmst1, 5), round(df1$rmst2, 5), 
                 round(sqrt(df1$vrmst1), 4), round(sqrt(df1$vrmst2), 4),
                 round(df1$rmstDiffZ^2, 4), 
                 round(2*min(df1$rmstDiffPValue, 1-df1$rmstDiffPValue), 4)),
               c(rmst1, rmst2, stderr1, stderr2, rmstdiffchisq, pvalue))
})


test_that("rmdiff: stratified test of rmst difference", {
  ...
  expect_equal(c(round(df1$rmstDiff, 4), round(sqrt(df1$vrmstDiff), 3), 
                 round(df1$rmstDiffZ, 3)),
               c(round(rmstDiff, 4), round(sqrt(vrmstDiff), 3),
                 round(rmstDiffZ, 3)))
})
