test_that("logisregr: freq works", {
  ...
  expect_equal(fit1$sumstat, fit2$sumstat)
  expect_equal(fit1$parest, fit2$parest)
})


test_that("logisregr: firth with plci", {
  ...
  expect_equal(round(fit1$parest$beta, 4), coef)
  expect_equal(round(fit1$parest$lower, 4), ci.lower)
  expect_equal(round(fit1$parest$upper, 4), ci.upper)
})


test_that("logisregr: firth with flic", {
  fit1 <- logisregr(sexagg, event="case", 
                    covariates=c("age", "oc", "vic", "vicl", "vis", "dia"), 
  ...
  expect_equal(round(fit1$parest$beta[1], 4), 0.1315)
})


test_that("logisregr: robust variance", {
  ...
  expect_equal(round(fit1$parest$beta, 4), coef)
  expect_equal(round(fit1$parest$sebeta, 4), stderr)
})
