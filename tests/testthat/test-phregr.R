...
test_that("phregr: handling ties", {
  ...
  for (ties in c("breslow", "efron")) {
    ...
    expect_equal(fit1$beta, fit2$coefficients)
...
test_that("phregr: counting process form", {
  ...
  expect_equal(as.numeric(fit1$beta), as.numeric(fit2$coefficients))
  dimnames(fit1$vbeta) <- NULL
  expect_equal(fit1$vbeta, fit2$var)
  expect_equal(c(fit1$sumstat$loglik0, fit1$sumstat$loglik1), fit2$loglik)
  expect_equal(fit1$sumstat$scoretest, fit2$score)
})


test_that("phregr: firth with plci", {
  ...
  expect_equal(round(c(fit1$parest$beta, fit1$parest$lower, 
                       fit1$parest$upper), 3), 
