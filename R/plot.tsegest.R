  ...
  if (x$settings$swtrt_control_only) {
    ...
    p_z1 <- ggplot2::ggplot(x$eval_z[[1]]$data, 
      ...
      ggplot2::geom_hline(
        ...
        linetype = linetype[yindex]) + 
      ggplot2::geom_vline(
        xintercept = xintercept1[xindex1], 
        linetype = linetype[xindex1]) + 
      ggplot2::ylim(limits) +
      ggplot2::labs(
        x = expression(paste("Causal parameter ", psi)), 
        y = expression(paste("Z-statistic ", Z(psi))),
    ...
    p_z2 <- ggplot2::ggplot(x$eval_z[[2]]$data, 
                            ggplot2::aes(x = .data$psi, y = .data$Z)) +
      ggplot2::geom_line() +
      ggplot2::geom_hline(
        yintercept = yintercept[yindex], 
        linetype = linetype[yindex]) + 
      ggplot2::geom_vline(
        xintercept = xintercept2[xindex2], 
        linetype = linetype[xindex2]) + 
      ggplot2::ylim(limits) +
      ggplot2::labs(
        x = expression(paste("Causal parameter ", psi)), 
        y = expression(paste("Z-statistic ", Z(psi))),
        title = df_arm$arm[[2]],
        caption = bquote(
          "Estimate of " * psi == .(sprintf("%.3f", x$psi_trt)) *
            " (" * .(sprintf("%.0f", conflev)) * "% CI: " *
            .(sprintf("%.3f", x$psi_trt_CI[1])) * ", " *
            .(sprintf("%.3f", x$psi_trt_CI[2])) * ")")) + 
      ggplot2::theme_bw() +
      ggplot2::theme(plot.caption = ggplot2::element_text(hjust = 0))
    
    p_z <- cowplot::plot_grid(plotlist = list(p_z1, p_z2), ncol = 2)
  }
